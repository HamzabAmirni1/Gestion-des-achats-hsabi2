...
import { 
  ...
  Download, Moon, Sun, RefreshCw, LogOut, Send, X
} from 'lucide-react';
...
export default function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    } catch (err) { Swal.fire('Erreur', err.message, 'error'); } finally { setIsSubmitting(false); }
  };
  ...
  if (!session) return (
    <div className="auth-page">
      <div className="auth-card-premium animate-enter">
        ...
        <p style={{marginBottom:25}}>Production مواد التنظيف</p>
        <form onSubmit={async (e)=>{e.preventDefault(); setAuthLoading(true); let r = authMode==='login' ? await supabase.auth.signInWithPassword({email:authEmail, password:authPassword}) : await supabase.auth.signUp({email:authEmail, password:authPassword}); if(r.error) setAuthError(r.error.message); setAuthLoading(false);}}>
           ...
           <button type="button" className="btn-google-auth" onClick={()=>supabase.auth.signInWithOAuth({provider:'google'})}>Utiliser Google</button>
  ...
  return (
    <div className="app-wrapper">
      <header className="header" id="header-el">
        <div className="header-title">Brasti platform {loading && '...'}</div>
      ...
      <main id="app-main-content">
        {activeTab==='dashboard' && (
          <div className="animate-enter">
            <div className="stats-grid">
               <div className="stat-card" style={{borderLeft:'5px solid #10b981'}}><div className="stat-value">{beneficeReel} DA</div><div className="stat-label">Bénéfice</div></div>
        ...
        {activeTab==='stock' && (
          <div className="animate-enter">
            <div className="section-header"><h3>Stock</h3><button className="btn-primary" onClick={()=>{setFormData({nom:'', quantite:1, prix:1, product_id:'', est_paye:true}); setModalType('product'); setIsModalOpen(true)}}>+ Nouveau</button></div>
        ...
        <div className="modal-overlay" onClick={()=>setIsModalOpen(false)}>
           <div className="modal-content" onClick={e=>e.stopPropagation()}>
              ...
              <form onSubmit={handleSubmit}>
                 <div className="form-group"><label>Nom</label><input className="form-control" value={formData.nom} required onChange={e=>setFormData({...formData, nom:e.target.value})} /></div>
                 {modalType === 'vente' && (
                    <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:10}}>
                       <div className="form-group"><label>Client</label><input className="form-control" value={formData.client_nom} placeholder="Optionnel" onChange={e=>setFormData({...formData, client_nom:e.target.value})} /></div>
