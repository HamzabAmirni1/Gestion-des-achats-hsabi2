...
export default function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setAchats(a.data || []); setVentes(v.data || []); setProducts(p.data || []);
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`Brasti_Rapport_${format(new Date(), 'yyyyMMdd_HHmm')}.pdf`);
      Swal.fire('OK', 'Rapport prêt !', 'success');
    ...
    <div className="auth-page">
      <div className="auth-card-premium animate-enter">
        ...
        <form onSubmit={async (e)=>{e.preventDefault(); setAuthLoading(true); let r = authMode==='login' ? await supabase.auth.signInWithPassword({email:authEmail, password:authPassword}) : await supabase.auth.signUp({email:authEmail, password:authPassword}); if(r.error) setAuthError(r.error.message); setAuthLoading(false);}}>
           ...
           {authError && <div style={{color:'red', fontSize:'0.8rem', margin:'10px 0'}}>{authError}</div>}
  ...
  return (
    <div className="app-wrapper" id="app-main-content">
      ...
      <main>
        {activeTab==='dashboard' && <div className="stats-grid animate-enter"><div className="stat-card" style={{borderLeft:'5px solid #10b981'}}><div className="stat-value">{benefice} DA</div><div className="stat-label">Bénéfice</div></div><div className="stat-card"><div className="stat-value">{totalVentes} DA</div><div className="stat-label">Ventes</div></div><div className="stat-card" style={{borderLeft:'5px solid #ef4444'}}><div className="stat-value">{totalAchats} DA</div><div className="stat-label">Dépenses</div></div></div>}
        {activeTab==='stock' && <div className="animate-enter"><div className="section-header"><h3>Gestion du Stock</h3><button className="btn-primary" onClick={()=>{setModalType('product'); setIsModalOpen(true)}}>+ Nouveau</button></div><div className="products-grid">{products.map(p=><div key={p.id} className="product-card"><b>{p.nom}</b> : <b>{p.stock_qty}</b> en stock</div>)}</div></div>}
        {activeTab==='ventes' && <div className="glass-container animate-enter"><div className="section-header"><h3>Historique de Ventes</h3><button className="btn-primary" onClick={()=>{setModalType('vente'); setIsModalOpen(true)}}>+ Vendre</button></div><table className="data-table"><thead><tr><th>Nom</th><th>Quantité</th><th>Total</th></tr></thead><tbody>{ventes.map(v=><tr key={v.id}><td>{v.nom}</td><td>{v.quantite}</td><td>{v.prix*v.quantite} DA</td></tr>)}</tbody></table></div>}
        {activeTab==='achats' && <div className="glass-container animate-enter"><div className="section-header"><h3>Historique d'Achats</h3><button className="btn-primary" onClick={()=>{setModalType('achat'); setIsModalOpen(true)}}>+ Acheter</button></div><table className="data-table"><thead><tr><th>Nom</th><th>Quantité</th><th>Total</th></tr></thead><tbody>{achats.map(a=><tr key={a.id}><td>{a.nom}</td><td>{a.quantite}</td><td>{a.prix*a.quantite} DA</td></tr>)}</tbody></table></div>}
      </main>
      {isModalOpen && (
        <div className="modal-overlay" onClick={()=>setIsModalOpen(false)}>
           <div className="modal-content" onClick={e=>e.stopPropagation()}>
              <div className="modal-header"><h3>{modalType==='product'?'Produit':(modalType==='achat'?'Achat':'Vente')}</h3><button onClick={()=>setIsModalOpen(false)}><X size={24}/></button></div>
              <form onSubmit={async (e)=>{e.preventDefault(); setIsSubmitting(true); const table=modalType==='product'?'products':(modalType==='achat'?'achats':'ventes'); const item={user_id:session.user.id, nom:formData.nom, prix:parseFloat(formData.prix), quantite:parseFloat(formData.quantite), product_id:formData.product_id||null, date:format(new Date(),'yyyy-MM-dd')}; await supabase.from(table).insert([item]); fetchData(); setIsModalOpen(false); setIsSubmitting(false);}}>
                 ...
                 {modalType !== 'product' && (
                    <div className="form-group"><label>Lier au Stock (Optionnel)</label><select className="form-control" onChange={e=>setFormData({...formData, product_id:e.target.value})}><option value="">Ne pas lier</option>{products.map(p=><option key={p.id} value={p.id}>{p.nom} (Actuel: {p.stock_qty})</option>)}</select></div>
                 )}
                 ...
                 <button type="submit" className="btn-primary" style={{width:'100%', marginTop:'10px'}} disabled={isSubmitting}>{isSubmitting?'...':'Confirmer & Sauvegarder'}</button>
              </form>
           </div>
        </div>
      )}
